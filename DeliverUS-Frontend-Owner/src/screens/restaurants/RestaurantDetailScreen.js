...
export default function RestaurantDetailScreen ({ navigation, route }) {
  ...
  // solucion
  const quedaPoco = (fechaLimite) => {
    const fechaActual = new Date()
    const fechaLimit = new Date(fechaLimite)

    const tiempoRestante = fechaActual.getTime() - fechaLimit.getTime()
  ...
  const renderProduct = ({ item }) => {
    return (
      ...
      >
        ...
        {/* solucion */}
        {item.visibleUntil !== null && quedaPoco(item.visibleUntil) &&
